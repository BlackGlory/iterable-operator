...
describe('AsyncIterableOperator<T>::reduceAsync(fn: (accumulator: T, currentValue: T, index: number) => T | PromiseLike<T>): Promise<T>', () => {
...
describe('AsyncIterableOperator<T>::reduceAsync<U>(fn: (accumulator: U, currentValue: T, index: number) => U | PromiseLike<U>, initalValue: U): Promise<U>', () => {
  describe('fn is called', () => {
    it('called with [accumulator,currentValue,index]', async () => {
      const iter = getIter([1, 2, 3])
      const fn = jest.fn()
